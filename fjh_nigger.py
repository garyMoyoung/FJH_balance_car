...
def send_data(uart, x, y, w, h):
    def to_uint16_bytes(val):
...
def process_blobs(img, blobs, color):
    ...
    for blob in blobs:
        ...
        w = blob[2]
        h = blob[3]
        send_data(uart, x, y, w, h)
        # 在左上角显示 w 和 h
        img.draw_string_advanced(0, 120, 30, f'x: {x}', color=(255,255,255))
        img.draw_string_advanced(0, 160, 30, f'y: {y}', color=(255,255,255))
        img.draw_string_advanced(0, 40, 30, f'w: {w}', color=(255,255,255))
        img.draw_string_advanced(0, 80, 30, f'h: {h}', color=(255,255,255))
        img.draw_string_advanced(0, 200, 30, f'S: {w*h}', color=(255,255,255))
